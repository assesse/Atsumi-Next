...
| internal duplicate | 완료 | verified artifact 내부 499p 상한, algorithm v4 bounded N-way scene clustering + edition track, set-level Review, CAS plan, page quarantine·undo·startup recovery |
...
- `InternalDuplicateSupervisor`: 일반 UI에서는 선택한 1~200개의 verified complete entry만 직접 조회·hash하고, 빈 선택이나 일부 부적격 대상을 전체 검사로 대체하지 않는다. 완료 시 stale group 해제도 검사한 gallery로 한정해 미선택 결과를 보존한다. 설정의 명시적 라이브러리 재구축만 전체 artifact 경로를 사용한다. worker는 현재 entry의 page hash 수와 bounded pair 비교 수를 휘발성 `InternalArtifactScanProgress`로 발행하며 `internal_duplicate_active_artifact`로 재수화할 수 있다. 이 진행 상태는 SQLite/schema에 저장하지 않고 검출 threshold나 결과 순서에 관여하지 않는다. 사용자가 고른 keep/remove source page와 현재 파일 수·byte 합계를 15분 계획으로 고정한다. page move는 DB intent 뒤 artifact 내부 `.atsumi-page-quarantine/<plan-id>/`로 수행하고 manifest atomic replace 뒤 DB state를 확정한다.
...
- `App.tsx`, typed backend와 `InternalDuplicateDialog.tsx`: Downloads에서 선택한 완료 앨범만 scan 시작·취소·진행·오류·재시도하고, 검토창의 재검사도 현재 entry 하나만 대상으로 한다. 현재 검사 중인 entry의 카드에 해당 artifact의 단계와 진행률을 표시하며, 미해결 결과가 있는 완료 카드에는 좌측 하단 `내부 검토 N` 바로가기를 표시한다. edition Review는 행을 판본 세트, 열을 장면으로 두고 각 세트 행의 맨 왼쪽 checkbox로 하나 이상의 유지 세트를 고른다. 선택 행들은 모두 유지하고 다른 행만 격리 예정으로 표시하며, 선택 세트가 모두 누락된 장면은 건드리지 않는다. dialog 외곽은 세로 스크롤만 사용하고 넓은 행렬만 자체 가로 스크롤한다. 파일 수·용량 계획 preview, 명시적 격리 적용과 이력 기반 undo를 연결했다. page 이미지는 live source가 아니라 전역 coordinator의 verified `artifactPage(entryId, sourcePage)`만 사용한다.
