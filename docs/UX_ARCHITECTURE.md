...
- 제목과 결과 요약
- context row 좌측에 전체/기간별/작가별과 전체 펼치기/접기를 배치하고, 우측 heading action에는 즐겨찾기 작가 갱신만 둔다.
- 전체는 평면 목록이고 기간별과 작가별 결과는 일별·작가별 아코디언으로 접을 수 있으며, 접힘 상태는 다음 실행에도 유지한다.
- 아코디언 header는 별도 box 대신 얇은 primary accent/하단 구분선, compact 제목, 개수 pill과 chevron을 사용한다. 카드 preview가 작아져도 header가 과도하게 커지지 않는다.
- 작가별 보기에서 작가 기준값은 정보 action으로 제공한다.
- 후보 카드의 구조와 선택 규칙은 Explore와 같다.

### Downloads

- context row 좌측에 전체/기간별/작가별과 전체 펼치기/접기를 Auto Find와 같은 순서로 배치한다. heading action에는 검사·다운로드 명령만 둔다.
- 전체는 평면 목록이고 기간별과 작가별 결과는 아코디언으로 접을 수 있으며, 접힘 상태는 다음 실행에도 유지한다.
- 상태 filter: 전체, 작업 중, 검토, 실패, 완료
- 카드에는 짧은 상태만 표시
- 상세 오류와 재시도는 카드 상태 badge 또는 Activity detail에서 연다.
- 완료 전 판본 겹침은 typed warning badge에서 전용 Review로 연다. 기존 보유 앨범과 새 다운로드를 명시적으로 구분하고, 사용자 결정 전에는 completed로 표시하지 않는다.
